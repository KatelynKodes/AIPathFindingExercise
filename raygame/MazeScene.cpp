...
Maze::Maze()
{
	...
	TileKey map[Maze::HEIGHT][Maze::WIDTH] = {
		...
		{ w, _, w, w, w, w, w, w, _, c, _, w, _, _, w, w, w, w, w, w, w, w, w, w, w, w, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, w, _, _, _, _, w, w, w, w, w, _, w, w, w, w, w, w },
		{ w, _, _, w, w, w, w, w, _, _, _, w, _, g, _, _, _, _, _, _, w, _, _, _, _, _, _, w },
		{ w, _, _, w, w, w, w, w, _, _, _, _, _, _, _, _, w, w, w, w, w, _, w, w, w, w, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w, w, w, w, w, _, w, w, w, w, _, w },
		{ w, w, w, w, w, w, w, _, _, _, _, _, _, _, _, _, _, _, _, _, w, _, w, w, w, w, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w, w, w, w, w, _,w, w, w, w, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w, _, w, w, w, w, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, w, w, _, _, w, w, _, w, w, w, _, _, _, _, _, _, w },
		{ w, w, _, w, w, w, w, w, _, w, w, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, _, _, p, _, _, _, _, _, _, _, _, _, _, c, _, _, w },
		{ w, _, _, c, _, _, _, _, _, _, w, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, _, _, _, _, _, _, _, _, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, w, w, w, w, w, w, w, _, w, _, _, _, _, _, _, w, w, w, w, w, w, w, w, w, _, w },
		{ w, _, w, w, w, w, w, w, w, _, _, _, _, _, _, _, w, w, w, w, w, w, w, w, w, w, _, w },
		{ w, _, _, _, _, _, _, _, _, _, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, _, _, _, _, _, _, _, _, w, _, _, _, _, _, w, w, w, w, w, w, w, w, w, w, _, w },
		{ w, _, _, _, _, _, _, _, _, _, w, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, w, w, w, w, w, w, w, _, _, _, _, _, _, _, w, w, w, w, w, w, w, w, w, w, _, w },
		{ w, _, w, w, w, w, w, w, w, _, w, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w, w, w, w, w, w, w, w, _, _, w, w, w },
		{ w, _, _, _, _, _, _, _, _, _, w, _, _, _, _, _, _, _, _, _, _, _, _, _, w, w, w, w },
		{ w, _, w, w, w, w, w, w, w, _, w, _, _, _, _, w, w, w, w, w, w, w, w, _, _, w, w, w },
		{ w, _, w, w, w, w, w, w, w, _, w, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, w },
		{ w, _, _, _, _, _, _, _, _, _, w, _, _, _, _, w, w, w, w, w, w, w, w, _, _, w, w, w },
		{ w, _, _, _, _, _, _, _, _, _, w, _, _, _, _, _, _, _, _, _, _, _, _, _, w, w, w, w },
		{ w, _, _, _, _, _, _, _, _, _, w, _, _, _, _, w, w, w, w, w, w, w, w, _, _, w, w, w },
		{ w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w, w },
	};
...
Maze::Tile Maze::createTile(int x, int y, TileKey key)
{
	...
	{
		tile.cost = 1.0f;
		Collectable* collectable = new Collectable(position.x, position.y, 100, 200, 0xFF6666FF, this);
	...
	{
		tile.cost = 1.0f;
		Ghost* ghost = new Ghost(position.x, position.y, 100, 200, 0xFF6666FF, this);
		ghost->setTarget();
		tile.actor = ghost;
		addActor(tile.actor);
		break;
